...
wau_ch = (sess.drop_duplicates(subset=["user_id","week_start"])  # distinct users per week
...
ord_ch = (ordu.groupby(["week_start","acquisition_channel"], as_index=False)
               .agg(orders_net=("is_refund", lambda s: (s == 0).sum()),
                    revenue_net=("revenue",  lambda s: s.where(ordu.loc[s.index, "is_refund"] == 0, 0).sum())))

# --- Join + compute OPAC ---
out = (wau_ch.merge(ord_ch, on=["week_start","acquisition_channel"], how="outer")
...
fmt = {"opac":"{:.4f}".format}
print(f"✅ saved {path}")
print(out[["week_start","acquisition_channel","wau","orders_net","opac"]]
