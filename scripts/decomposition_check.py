...
df = wau.merge(
    opac[["week_start", "opac", "orders_net", "revenue_net"]], on="week_start", how="outer"
).merge(
    aov[["week_start", "aov", "orders_net", "revenue_net"]].rename(
        columns={"orders_net": "orders_net_aov", "revenue_net": "revenue_net_aov"}
    ),
    on="week_start",
    how="outer",
)
