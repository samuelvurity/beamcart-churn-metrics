...
df = pd.read_csv(interim/"refund_rate_by_week.csv", parse_dates=["week_start"]).sort_values("week_start")
if df.empty: raise SystemExit("No refund data. Run pipeline first.")
last = df.iloc[-1]
...
plt.xlabel("ISO Week Start (UTC)"); plt.ylabel("Refund Rate"); plt.grid(True, linewidth=0.4); plt.tight_layout()
out = charts/"refund_rate_trend.png"; plt.savefig(out, dpi=160)
