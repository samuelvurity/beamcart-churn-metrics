...
events = [
    ...
    ("u2", (BASE + timedelta(days=12, hours=12)).strftime("%Y-%m-%d %H:%M:%S"), S),   # week 2 (churns after)
    ...
    ("u4", (BASE + timedelta(days=11, hours=12)).strftime("%Y-%m-%d %H:%M:%S"), S),   # week 2 only (then churn)
    # u4 no week 3

    ("u5", (BASE + timedelta(days=15, hours=10)).strftime("%Y-%m-%d %H:%M:%S"), S),   # week 3 (activated within 7d)
]

# Orders: include one refund to test guardrail; exclude refunds in AOV/OPAC
orders = [
    # order_id, user_id, order_ts, revenue, items, is_refund
    ("o1", "u1", (BASE + timedelta(days=7,  hours=12, minutes=5)).strftime("%Y-%m-%d %H:%M:%S"), 40.00, 1, 0),
    ("o2", "u2", (BASE + timedelta(days=4,  hours=12, minutes=30)).strftime("%Y-%m-%d %H:%M:%S"), 35.00, 1, 1),  # refund
    ("o3", "u3", (BASE + timedelta(days=8,  hours=10)).strftime("%Y-%m-%d %H:%M:%S"), 55.00, 2, 0),
    ("o4", "u4", (BASE + timedelta(days=11, hours=13, minutes=30)).strftime("%Y-%m-%d %H:%M:%S"), 20.00, 1, 0),
    ("o5", "u1", (BASE + timedelta(days=14, hours=13)).strftime("%Y-%m-%d %H:%M:%S"), 42.00, 1, 0),
    ("o6", "u3", (BASE + timedelta(days=14, hours=13, minutes=30)).strftime("%Y-%m-%d %H:%M:%S"), 30.00, 1, 0),
]

write_csv(os.path.join(RAW, "users.csv"),
          ["user_id","signup_ts","country","acquisition_channel"],
          users)

write_csv(os.path.join(RAW, "events.csv"),
          ["user_id","event_ts","event_type"],
          events)

write_csv(os.path.join(RAW, "orders.csv"),
          ["order_id","user_id","order_ts","revenue","items","is_refund"],
          orders)
