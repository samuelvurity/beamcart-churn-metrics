...
agg = orders.groupby("week_start", as_index=False).agg(
    all_orders=("order_id", "count"), refund_orders=("is_refund", lambda s: (s == 1).sum())
)
...
print(
    agg.to_string(
        index=False, formatters={"refund_rate": (lambda v: "NaN" if pd.isna(v) else f"{v:.4f}")}
    )
)
