...
sql_ret = con.execute(sql).df()   # signup_date, cohort_size, d1_retention, d7_retention, d30_retention
sql_ret["signup_date"] = pd.to_datetime(sql_ret["signup_date"])

# save for reference
(sql_ret.sort_values("signup_date")
        .to_csv(INTERIM / "retention_summary_sql.csv", index=False))

# pandas version
py_ret = pd.read_csv(INTERIM / "retention_summary.csv", parse_dates=["signup_date"])

merged = (py_ret.merge(sql_ret, on="signup_date", suffixes=("_py","_sql"))
...
print(sql_ret.to_string(index=False, 
                        formatters={"d1_retention":"{:.4f}".format,
                                    "d7_retention":"{:.4f}".format,
                                    "d30_retention":"{:.4f}".format}))
print("\nParity vs pandas:")
cols = ["signup_date","cohort_size_py","cohort_size_sql",
        "d1_retention_py","d1_retention_sql","d1_match",
        "d7_retention_py","d7_retention_sql","d7_match",
        "d30_retention_py","d30_retention_sql","d30_match"]
...
if "cohort_size_py" not in merged.columns:
    ...
    merged = merged.merge(d1[["signup_date","cohort_size"]]
                          .rename(columns={"cohort_size":"cohort_size_py"}),
                          on="signup_date", how="left")
