...
merged = (py_aov.merge(sql_aov, on="week_start", how="outer", suffixes=("_py", "_sql"))
...
merged["orders_match"]  = (merged["orders_net_py"].fillna(-1).astype(float) == merged["orders_net_sql"].fillna(-1).astype(float))
...
print("\nParity vs pandas:")
cols = ["week_start","orders_net_py","orders_net_sql","revenue_net_py","revenue_net_sql","aov_py","aov_sql","orders_match","revenue_match","aov_match"]
